   ...
   */
     ...
     */
        explicit ConverterParams(const bool pHasDefaultConverter = false,
            ...
            : mHasDefaultConverter(pHasDefaultConverter), mDefaultFloat(pDefaultFloat), mDefaultInteger(pDefaultInteger)
        {
        }
   ...
   */
     ...
     */
        explicit SeparatorParams(const char pSeparator = ',', const bool pTrim = false,
            ...
            : mSeparator(pSeparator), mTrim(pTrim), mHasCR(pHasCR), mQuotedLinebreaks(pQuotedLinebreaks), mAutoQuote(pAutoQuote)
        {
        }
   ...
   */
     ...
     */
        explicit Document(const std::string &pPath = std::string(),
            ...
            : mPath(pPath), mLabelParams(pLabelParams), mSeparatorParams(pSeparatorParams), mConverterParams(pConverterParams), mLineReaderParams(pLineReaderParams)
        {
            if (!mPath.empty())
            {
                ReadCsv();
     ...
     */
        explicit Document(std::istream &pStream,
                          ...
                          const ConverterParams &pConverterParams = ConverterParams(),
                          const LineReaderParams &pLineReaderParams = LineReaderParams())
            : mPath(), mLabelParams(pLabelParams), mSeparatorParams(pSeparatorParams), mConverterParams(pConverterParams), mLineReaderParams(pLineReaderParams)
        {
            ReadCsv(pStream);
        }
