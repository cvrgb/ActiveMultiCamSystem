...
private:
					  ...
					  Params &params_)
	{
		...
		for (const auto &rt : v_rt_)
		{
			...
			Eigen::Matrix<double, 3, 4> rrt;
			rrt.block<3, 3>(0, 0) = r_v.toRotationMatrix();
			rrt.block<3, 1>(0, 3) = Eigen::Vector3d(rt(3), rt(4), rt(5));
			params_.vec_rt.push_back(rrt);
		}
	}
	...
	struct ReprojectionError
	{
						...
						T *residuls) const
		{
			...
			T p1 = k_[3];
			T p2 = k_[4];
			T r_coeff = T(1) * (T(1) + k_[0] * r2 + k_[1] * r4 + k_[2] * r6);
			T xd = c_x * r_coeff + 2.0 * p1 * c_x * c_y + p2 * (r2 + 2.0 * c_x * c_x);
			T yd = c_y * r_coeff + 2.0 * p2 * c_x * c_y + p1 * (r2 + 2.0 * c_y * c_y);
			;
