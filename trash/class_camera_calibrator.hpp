...
{
public:
	void set_input(const std::vector<cv::Mat> &vec_mat_, const cv::Size &chessboard_size_)
	{
		_boards_pts.clear();
		_imgs_pts.clear();
		int cc = 0;
		std::vector<Eigen::MatrixX2d> views_image_p2ds;
		std::vector<Eigen::MatrixX2d> views_board_p2ds;
		Eigen::MatrixX2d bp = CalibrateCamera::createCheckerboardPoints(
			Eigen::Vector2i(chessboard_size_.width, chessboard_size_.height), 1.0);
		...
		for (const auto &img : vec_mat_)
		{
			...
			Eigen::MatrixX2d image_p2ds;
			image_p2ds.resize(11 * 8, 2);
			image_p2ds.setZero();
			for (int r = 0; r < corner_pts.size(); ++r)
			{

				image_p2ds(r, 0) = (corner_pts)[r].x;
				image_p2ds(r, 1) = (corner_pts)[r].y;
			}

			views_board_p2ds.push_back(bp);
			views_image_p2ds.push_back(image_p2ds);
		}
		printf("# [DEBUG] NUMBER OF VALID OBSERVATIONS %d\n", cc);
		Eigen::Matrix3d K;
		Eigen::VectorXd D;
		D.resize(5);
		D.setZero();
		CalibrateCamera::calibrateCamera(views_image_p2ds, views_board_p2ds, K, D,
										 CalibrateCamera::CalibFlags::MODEL_K3P2_MULTIPLY);
		printf("# [DEBUG] ===============================\n");
		std::cout << K << std::endl;
		std::cout << D << std::endl;
		printf("# [DEBUG] ===============================\n");
					...
					std::vector<Eigen::MatrixXd> &vec_extrinsics_)
	{
		...
		// camera_matrix_(0, 0) = 3510;
		// camera_matrix_(1, 1) = 1008;
		// camera_matrix_(1, 2) = 686;
...
private:
	void make_board_points(const cv::Size &chessboard_size_)
	{
		...
		for (int r = 0; r < chessboard_size_.height; ++r)
		{
			...
			{
				vec_points.emplace_back(c * 1.0, r * 1.0);
			}
		}
	...
	void get_homography(std::vector<Eigen::Matrix3d> &vec_h_)
	{
		...
		for (int i = 0; i < _imgs_pts.size(); ++i)
		{
			...
			//std::cout << "====================================== ini_H" << std::endl;
			//std::cout << ini_H << std::endl;
			//std::cout << "--------------------------------------" << std::endl;
			// Eigen::MatrixX2d board_p2ds;
			// Eigen::MatrixX2d image_p2ds;
			// board_p2ds.resize(11 * 8, 2);
			// board_p2ds.setZero();
			// image_p2ds.resize(11 * 8, 2);
			// image_p2ds.setZero();
			// for (int r = 0; r < _imgs_pts[i].size(); ++r)
			// {
			// 	board_p2ds(r, 0) = (_boards_pts[i])[r].x;
			// 	board_p2ds(r, 1) = (_boards_pts[i])[r].y;
			// 	image_p2ds(r, 0) = (_imgs_pts[i])[r].x;
			// 	image_p2ds(r, 1) = (_imgs_pts[i])[r].y;
			// }
			// CalibrateCamera::estimatePlanarHomographyMatrix(image_p2ds, board_p2ds, ini_H);

			//std::cout << "-------------------------------------- refined_H" << std::endl;
			optimier.refine_H(_imgs_pts[i], _boards_pts[i], ini_H, refined_H);
			//std::cout << refined_H << std::endl;
			// CalibrateCamera::RefineHomographyOptions options;
			// options.expected_average_symmetric_distance = 0.02;
			// Eigen::MatrixXd x0 = board_p2ds.transpose().eval();
			// Eigen::MatrixXd x1 = image_p2ds.transpose().eval();
			// CalibrateCamera::EstimateHomography2DFromCorrespondences(x0, x1, options, &ini_H);
			// std::cout << ini_H << std::endl;
